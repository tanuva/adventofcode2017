...
func toArray(_ s: String) -> [[Int]] {
	let colSeparator: Character = s.contains(" ") ? " " : "\t"
	var rows: [[Int]] = []
	var row: [Int] = []

	for line in s.split(separator: "\n") {
		for column in line.split(separator: colSeparator) {
			guard let val = Int(column) else { continue }
			row.append(val)
		}
	...
	for row in data {
		for a in row {
			for b in row {
				let dA = Double(a)
				let dB = Double(b)
				if a != b && dA.remainder(dividingBy: dB).isZero {
					sum += Int(dA / dB)
				}
			}
		}
...
"""]
	...
	for input in inputs {
		print("\(inputs.index(of: input)!): \(solve2(toArray(input)))")
	}
}
